...
entity Pong_Top2 is
  ...
  port (
    ...
    o_Ball_Data : out std_logic_vector(31 downto 0);
...
architecture rtl of Pong_Top2 is
  ...
  signal w_P1_Digit : integer range 0 to 9 := 0;
  signal w_P2_Digit : integer range 0 to 9 := 0;

  function f_7seg_active_low(i_digit : integer) return std_logic_vector is
    variable v_seg : std_logic_vector(7 downto 0);
  begin
    case i_digit is
      when 0 => v_seg := "11000000";
      when 1 => v_seg := "11111001";
      when 2 => v_seg := "10100100";
      when 3 => v_seg := "10110000";
      when 4 => v_seg := "10011001";
      when 5 => v_seg := "10010010";
      when 6 => v_seg := "10000010";
      when 7 => v_seg := "11111000";
      when 8 => v_seg := "10000000";
      when 9 => v_seg := "10010000";
      when others => v_seg := "11111111";
    end case;
...
--  end process p_Reg_Syncs; 
 ...
 o_Ball_Data <= std_logic_vector(resize(unsigned(w_Ball_X), 16)) & std_logic_vector(resize(unsigned(w_Ball_Y), 16));
  ...
  w_P1_Digit <= r_P1_Score rem 10;
  w_P2_Digit <= r_P2_Score rem 10;

  -- Show each score on a single digit of each 7-seg display.
  D0_AN  <= "1110"; -- enable digit 0 only (active low)
  D1_AN  <= "1110"; -- enable digit 0 only (active low)
  D0_SEG <= f_7seg_active_low(w_P1_Digit);
  D1_SEG <= f_7seg_active_low(w_P2_Digit);
