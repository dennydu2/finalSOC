// AES-128 pure software implementation (FIPS 197)
// Table-driven: uses forward and inverse S-boxes
#include "aes128.h"
#include <string.h>

// ---- S-boxes ----
static const uint8_t SBOX[256] = {
  
};

static const uint8_t SBOX_INV[256] = {
  
};

// GF(2^8) multiply by 2 (xtime)
static inline uint8_t xtime(uint8_t a) {
  return (a << 1) ^ (a & 0x80 ? 0x1b : 0);
}

static inline uint8_t gmul(uint8_t a, uint8_t b) {
  uint8_t r = 0;
  while (b) {
    if (b & 1) r ^= a;
    a = xtime(a);
    b >>= 1;
  }
  return r;
}

// Key expansion: produces 44 32-bit round key words
static void key_expand(const uint8_t key[16], uint32_t rk[44]) {
  static const uint8_t rcon[10] = {0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36};
  int i;
  for (i = 0; i < 4; i++) {
    rk[i] = ((uint32_t)key[4*i]   << 24) | ((uint32_t)key[4*i+1] << 16) |
             ((uint32_t)key[4*i+2] <<  8) |  (uint32_t)key[4*i+3];
  }
  for (i = 4; i < 44; i++) {
    uint32_t t = rk[i-1];
    if (i % 4 == 0) {
      // SubWord(RotWord(t)) XOR Rcon
      t = (t << 8) | (t >> 24); // RotWord
      t = ((uint32_t)SBOX[(t>>24)&0xff] << 24) | ((uint32_t)SBOX[(t>>16)&0xff] << 16) |
          ((uint32_t)SBOX[(t>> 8)&0xff] <<  8) |  (uint32_t)SBOX[t & 0xff];
      t ^= (uint32_t)rcon[i/4-1] << 24;
    }
    rk[i] = rk[i-4] ^ t;
  }
}

// State: s[row][col], row-major byte order
static void add_round_key(uint8_t s[4][4], const uint32_t *rk) {
  int c;
  for (c = 0; c < 4; c++) {
    s[0][c] ^= (rk[c] >> 24) & 0xff;
    s[1][c] ^= (rk[c] >> 16) & 0xff;
    s[2][c] ^= (rk[c] >>  8) & 0xff;
    s[3][c] ^= (rk[c]      ) & 0xff;
  }
}

static void sub_bytes(uint8_t s[4][4]) {
  int r, c;
  for (r = 0; r < 4; r++) for (c = 0; c < 4; c++) s[r][c] = SBOX[s[r][c]];
}

static void sub_bytes_inv(uint8_t s[4][4]) {
  int r, c;
  for (r = 0; r < 4; r++) for (c = 0; c < 4; c++) s[r][c] = SBOX_INV[s[r][c]];
}

static void shift_rows(uint8_t s[4][4]) {
  uint8_t t;
  // row 1: shift left 1
  t=s[1][0]; s[1][0]=s[1][1]; s[1][1]=s[1][2]; s[1][2]=s[1][3]; s[1][3]=t;
  // row 2: shift left 2
  t=s[2][0]; s[2][0]=s[2][2]; s[2][2]=t; t=s[2][1]; s[2][1]=s[2][3]; s[2][3]=t;
  // row 3: shift left 3
  t=s[3][3]; s[3][3]=s[3][2]; s[3][2]=s[3][1]; s[3][1]=s[3][0]; s[3][0]=t;
}

static void shift_rows_inv(uint8_t s[4][4]) {
  uint8_t t;
  t=s[1][3]; s[1][3]=s[1][2]; s[1][2]=s[1][1]; s[1][1]=s[1][0]; s[1][0]=t;
  t=s[2][0]; s[2][0]=s[2][2]; s[2][2]=t; t=s[2][1]; s[2][1]=s[2][3]; s[2][3]=t;
  t=s[3][0]; s[3][0]=s[3][1]; s[3][1]=s[3][2]; s[3][2]=s[3][3]; s[3][3]=t;
}

static void mix_columns(uint8_t s[4][4]) {
  int c;
  for (c = 0; c < 4; c++) {
    uint8_t a0=s[0][c], a1=s[1][c], a2=s[2][c], a3=s[3][c];
    s[0][c] = gmul(a0,2)^gmul(a1,3)^a2^a3;
    s[1][c] = a0^gmul(a1,2)^gmul(a2,3)^a3;
    s[2][c] = a0^a1^gmul(a2,2)^gmul(a3,3);
    s[3][c] = gmul(a0,3)^a1^a2^gmul(a3,2);
  }
}

static void mix_columns_inv(uint8_t s[4][4]) {
  int c;
  for (c = 0; c < 4; c++) {
    uint8_t a0=s[0][c], a1=s[1][c], a2=s[2][c], a3=s[3][c];
    s[0][c] = gmul(a0,0xe)^gmul(a1,0xb)^gmul(a2,0xd)^gmul(a3,0x9);
    s[1][c] = gmul(a0,0x9)^gmul(a1,0xe)^gmul(a2,0xb)^gmul(a3,0xd);
    s[2][c] = gmul(a0,0xd)^gmul(a1,0x9)^gmul(a2,0xe)^gmul(a3,0xb);
    s[3][c] = gmul(a0,0xb)^gmul(a1,0xd)^gmul(a2,0x9)^gmul(a3,0xe);
  }
}

void aes128_enc(const uint8_t key[16], const uint8_t pt[16], uint8_t ct[16]) {
  uint32_t rk[44];
  uint8_t s[4][4];
  int r, c;

  key_expand(key, rk);

  // Load state (column-major: AES spec)
  for (r = 0; r < 4; r++) for (c = 0; c < 4; c++) s[r][c] = pt[r + 4*c];

  add_round_key(s, rk);
  for (int round = 1; round <= 10; round++) {
    sub_bytes(s);
    shift_rows(s);
    if (round < 10) mix_columns(s);
    add_round_key(s, rk + 4*round);
  }

  for (r = 0; r < 4; r++) for (c = 0; c < 4; c++) ct[r + 4*c] = s[r][c];
}

void aes128_dec(const uint8_t key[16], const uint8_t ct[16], uint8_t pt[16]) {
  uint32_t rk[44];
  uint8_t s[4][4];
  int r, c;

  key_expand(key, rk);

  for (r = 0; r < 4; r++) for (c = 0; c < 4; c++) s[r][c] = ct[r + 4*c];

  add_round_key(s, rk + 40);
  for (int round = 9; round >= 0; round--) {
    shift_rows_inv(s);
    sub_bytes_inv(s);
    add_round_key(s, rk + 4*round);
    if (round > 0) mix_columns_inv(s);
  }

  for (r = 0; r < 4; r++) for (c = 0; c < 4; c++) pt[r + 4*c] = s[r][c];
}
